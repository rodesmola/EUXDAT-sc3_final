...
export default {
    ...
    methods: {
        getKeyCloakData(agroClimaticData, deploymentName){
            ...
            this.$http.post(this.platformURL.concat('/auth/realms/euxdat/protocol/openid-connect/token'), 
                ...
                this.$http.get(this.platformURL.concat('/auth/realms/euxdat/protocol/openid-connect/userinfo'), 
                    ...
                    this.createInputObject(agroClimaticData, deploymentName, response.body)
        ...
        createInputObject(agroClimaticData, deploymentName, authData){
    ...
    created(){
        this.$eventBus.$on('run-hpc-kc', (data)  => {      
            this.getKeyCloakData(data, this.$store.state.user.preferred_username.slice(2) + '-' + Date.now().toString());            
        });
    }
};
</script>
